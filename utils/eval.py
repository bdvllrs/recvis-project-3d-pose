...
def get_data_sequence(batch, device, hg_model, model, images, joints_2d, joints_3d):
    ...
    # poses_3d = un_normalize_data(poses_3d, data_mean,
    #                              data_std, dim_to_ignore_3d)
    # poses_3d = poses_3d.reshape(poses_3d.shape[0], 16, 3)
    joints_3d.append(poses_3d)


def get_data_human(batch, device, human_dataset, model, images, joints_2d, joints_3d):
    data_2d, data_3d, root_position, keys = batch
    predicted_3d = model(data_2d.to(device, torch.float)).detach().cpu().numpy()
    data_2d_un = un_normalize_data(data_2d, human_dataset.data_mean_2d,
