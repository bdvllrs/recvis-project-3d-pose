...
def show3Dpose(channels, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):  # blue, orange
    ...
    assert channels.size == len(
        data.H36M_NAMES) * 3, "channels should have 96 entries, it has %d instead" % channels.size
    vals = np.reshape(channels, (len(data.H36M_NAMES), -1))
...
def show2Dpose(channels, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):
    ...
    assert channels.shape[0] == len(
        data.H36M_NAMES) * 2, "channels should have 64 entries, it has %d instead" % channels.shape[0]
    vals = np.reshape(channels, (len(data.H36M_NAMES), -1))
